...
Loading modules and device properties
-------------------------------------
...
A ``Makefile`` is often helpful here, as sometimes there are external libraries or flags that need to be passed to the CUDA program. An example will be discussed later in :ref:`External libraries compiled using Makefiles`.   
